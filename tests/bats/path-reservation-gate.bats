...
@test "T1: a foreign exclusive lease blocks an Edit, exit exactly 2" {
  ...
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/skills/feature-implement/SKILL.md") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") FFS_RUN_ID=other bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 2 ]
...
@test "T1: block message is RUNNABLE — real generation, held_by matches, no placeholders, release actually works" {
  ...
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/skills/feature-implement/SKILL.md") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") FFS_RUN_ID=someone-else bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 0 ]
}
...
@test "T1: sanctioned exits survive the handler — plain ALLOW and plain BLOCK, enforce and audit" {
  ...
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/docs/pipeline.md") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") FFS_RUN_ID=other bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 0 ]
  ...
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/docs/pipeline.md") | FFS_COORD_MODE=audit CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") FFS_RUN_ID=other bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 0 ]
  [[ "$output" != *"COORD-GATE-FAIL"* ]]
  [[ "$output" != *"COORD-AUDIT"* ]]

  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/skills/feature-implement/SKILL.md") | FFS_COORD_MODE=audit CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") FFS_RUN_ID=other bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 0 ]
  [[ "$output" == *"COORD-AUDIT"* ]]
  [[ "$output" == *"PATH-RESERVED"* ]]
}
...
@test "T1: an uncovered path exits 0 with empty stderr" {
  ...
  [ "$status" -eq 0 ]
  mkdir -p "$REPO/docs"
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/docs/pipeline.md") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") FFS_RUN_ID=other bash $(printf '%q' "$HOOK") 2>&1 1>/dev/null"
  [ "$status" -eq 0 ]
  [ -z "$output" ]
}
...
PYEOF
  run bash -c "FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") FFS_RUN_ID=other bash $(printf '%q' "$HOOK") < $(printf '%q' "$BATS_TEST_TMPDIR/big-envelope.json")"
  [ "$status" -eq 2 ]
}
...
@test "T1: the envelope reaches json.load(sys.stdin) — store-present, non-Edit event exits 0, no COORD-GATE-FAIL" {
  ...
  [ "$status" -eq 0 ]
  run bash -c "printf '%s' '{\"tool_name\":\"Bash\",\"tool_input\":{\"command\":\"ls\"}}' | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 0 ]
  [[ "$output" != *"COORD-GATE-FAIL"* ]]
}

@test "T1: no store at all exits 0 in enforce mode" {
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/docs/a.md") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 0 ]
}
...
@test "T1: FFS_COORD_MODE=off exits 0 with a live blocking lease" {
  ...
  [ "$status" -eq 0 ]
  mkdir -p "$REPO/skills/feature-implement"
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/skills/feature-implement/SKILL.md") | FFS_COORD_MODE=off CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 0 ]
}
...
@test "P-11: not a git repo at all -> exit 0, marker-stub proof not required" {
  ...
  mkdir -p "$OUTSIDE"
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$OUTSIDE/x.md") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$OUTSIDE") bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 0 ]
}
...
@test "P-10/P-11: the delegation path still blocks — empty argv[2], invoked from OUTSIDE the repo" {
  ...
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/skills/feature-implement/SKILL.md") | PATH_RESERVATION_GATE_FORCE_DELEGATE=1 FFS_COORD_MODE=enforce FFS_RUN_ID=other bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 2 ]
  [[ "$output" == *"${holder_sid:0:8}"* ]]
  [[ "$output" != *"$holder_sid"* ]]
  [[ "$output" != *"COORD-GATE-FAIL"* ]]
  ...
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/docs/pipeline.md") | PATH_RESERVATION_GATE_FORCE_DELEGATE=1 FFS_COORD_MODE=enforce FFS_RUN_ID=other bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 0 ]
  [ -z "$output" ]
}
...
@test "P-12: no session minting — five guard invocations add zero files to sessions/" {
  ...
  for i in 1 2 3 4 5; do
    run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/other-$i.md") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") bash $(printf '%q' "$HOOK")"
  done
...
@test "P-12: own-session exemption via the run pointer" {
  ...
  [ "$status" -eq 0 ]
  mkdir -p "$REPO/skills/feature-implement"
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/skills/feature-implement/SKILL.md") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") FFS_RUN_ID=holder bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 0 ]
}
...
@test "P-12: own-session exemption via FFS_COORD_SESSION" {
  ...
  holder_sid="${lines[0]#session=}"
  mkdir -p "$REPO/skills/feature-implement"
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/skills/feature-implement/SKILL.md") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") FFS_COORD_SESSION=$holder_sid bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 0 ]
}
...
@test "P-12: unresolvable identity blocks and the message says how to recover WITHOUT leaking the impersonation token" {
  ...
  holder_sid="${lines[0]#session=}"
  mkdir -p "$REPO/skills/feature-implement"
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/skills/feature-implement/SKILL.md") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") FFS_RUN_ID=nobody bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 2 ]
...
@test "REQ-07: a shared entry with two foreign holders still exits 0" {
  ...
  [ "$status" -eq 0 ]
  mkdir -p "$REPO/docs"
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/docs/pipeline.md") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") FFS_RUN_ID=c bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 0 ]
}
...
@test "REQ-07: a shared co-holder elsewhere is not exempt from a DIFFERENT session's unrelated exclusive lease" {
  ...
  [ "$status" -eq 0 ]
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/docs/x.md") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") FFS_RUN_ID=a bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 2 ]
}
...
@test "REQ-07: overlap forms — prefix blocks descendant, exact blocks only itself, prefix does not block itself as a bare file, segment-wise not char-prefix" {
  ...
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/skills/x/b.md") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") FFS_RUN_ID=other bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 2 ]

  run acquire_as b "path:docs/a.md" exclusive
  [ "$status" -eq 0 ]
  mkdir -p "$REPO/docs"
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/docs/a.md.bak") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") FFS_RUN_ID=other bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 0 ]
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/docs/a.md/x") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") FFS_RUN_ID=other bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 0 ]

  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/skills") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") FFS_RUN_ID=other bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 0 ]

  run acquire_as c "path:ab/**" exclusive
  [ "$status" -eq 0 ]
  mkdir -p "$REPO/a"
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/a/b.md") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") FFS_RUN_ID=other bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 0 ]
}
...
@test "REQ-07/P-07: casefold blocks including the non-ASCII pair" {
  ...
  [ "$status" -eq 0 ]
  mkdir -p "$REPO/docs"
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/docs/a.md") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") FFS_RUN_ID=other bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 2 ]
  ...
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/$strasse_upper/x.md") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") FFS_RUN_ID=other bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 2 ]
}
...
@test "REQ-07/P-19: NFC/NFD equivalence, both directions" {
  ...
  [ "$status" -eq 0 ]
  mkdir -p "$REPO/$nfc"
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/$nfc/x.md") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") FFS_RUN_ID=other bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 2 ]
}
...
@test "P-19: a symlinked edit path resolves into the lease and is blocked" {
  ...
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/shortcut.md") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") FFS_RUN_ID=other bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 2 ]
}
...
@test "P-19: a glob-charset filename and an embedded-newline filename are both blocked by a covering prefix lease" {
  ...
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/docs/a[1].md") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") FFS_RUN_ID=other bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 2 ]

  nl_name="docs/a"$'\n'"b.md"
  touch "$REPO/$nl_name"
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/$nl_name") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") FFS_RUN_ID=other bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 2 ]
}
...
@test "P-19: a symlink escaping outward exits 0 with no traceback" {
  ...
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/out.md") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") FFS_RUN_ID=other bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 0 ]
  [[ "$output" != *"Traceback"* ]]
}
...
@test "mode precedence: env beats file; file alone; neither -> enforce" {
  ...
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/skills/feature-implement/SKILL.md") | CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") FFS_RUN_ID=other bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 0 ]
  [[ "$output" == *"COORD-AUDIT"* ]]
...
@test "an invalid mode value (env and file) is treated as enforce, quoting the offending value" {
  ...
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/docs/a.md") | CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") FFS_RUN_ID=other bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 2 ]
  rm -f "$STORE/mode"
}
...
@test "REQ-08: P-21 envelope taxonomy — malformed rows fail per mode, benign rows always pass" {
  ...
  for m in "${malformed[@]}"; do
    run bash -c "printf '%s' $(printf '%q' "$m") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") bash $(printf '%q' "$HOOK")"
    [ "$status" -eq 2 ]
    [[ "$output" == *"COORD-GATE-FAIL"* ]]
    run bash -c "printf '%s' $(printf '%q' "$m") | FFS_COORD_MODE=audit CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") bash $(printf '%q' "$HOOK")"
    [ "$status" -eq 0 ]
    [[ "$output" == *"COORD-AUDIT"* ]]
  done
  ...
  for b in "${benign[@]}"; do
    run bash -c "printf '%s' $(printf '%q' "$b") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") bash $(printf '%q' "$HOOK")"
    [ "$status" -eq 0 ]
    [[ "$output" != *"COORD-GATE-FAIL"* ]]
    [[ "$output" != *"COORD-AUDIT"* ]]
    run bash -c "printf '%s' $(printf '%q' "$b") | FFS_COORD_MODE=audit CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") bash $(printf '%q' "$HOOK")"
    [ "$status" -eq 0 ]
    [[ "$output" != *"COORD-GATE-FAIL"* ]]
    [[ "$output" != *"COORD-AUDIT"* ]]
  done
...
@test "REQ-08: the discriminator pin — Bash+non-dict tool_input passes, Edit+non-dict tool_input fails" {
  run acquire_as holder "path:docs/**" exclusive
  [ "$status" -eq 0 ]
  run bash -c "printf '%s' '{\"tool_name\":\"Bash\",\"tool_input\":\"nope\"}' | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 0 ]
  run bash -c "printf '%s' '{\"tool_name\":\"Edit\",\"tool_input\":\"nope\"}' | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 2 ]
}
...
@test "REQ-08: unreadable store shape (i) — unparseable registry.json, enforce 2 / audit warns via coord's own WARNING line" {
  ...
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/docs/a.md") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 2 ]
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/docs/a.md") | FFS_COORD_MODE=audit CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 0 ]
...
@test "REQ-08: the SAME chmod 000 store on the DELEGATION path — enforce 2 / audit 0, never 'not a git repository'" {
  ...
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/docs/a.md") | PATH_RESERVATION_GATE_FORCE_DELEGATE=1 FFS_COORD_MODE=enforce bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 2 ]
  [[ "$output" != *"not a git repository"* ]]

  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/docs/a.md") | PATH_RESERVATION_GATE_FORCE_DELEGATE=1 FFS_COORD_MODE=audit bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 0 ]
...
@test "REQ-08/P-16: filelock import failure exits 2 in enforce, cold and warm cache" {
  ...
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/other/x.md") | PYTHONPATH=$(printf '%q' "$FAKELIB") FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 2 ]
  ...
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/other/x.md") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 0 ]
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/other/x.md") | PYTHONPATH=$(printf '%q' "$FAKELIB") FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 2 ]
}
...
@test "REQ-08/P-16: filelock failure in audit, via BOTH the env-mode route and the store-mode-file-only route" {
  ...
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/docs/a.md") | PYTHONPATH=$(printf '%q' "$FAKELIB") FFS_COORD_MODE=audit CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 0 ]

  printf 'audit' >"$STORE/mode"
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/docs/a.md") | PYTHONPATH=$(printf '%q' "$FAKELIB") CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 0 ]
  rm -f "$STORE/mode"
}
...
@test "REQ-08: non-Edit-family events pass in every mode with a live blocking lease" {
  ...
  for tool in Bash Read Grep NotebookEdit; do
    run bash -c "$(declare -f envelope); envelope $tool $(printf '%q' "$REPO/docs/a.md") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") bash $(printf '%q' "$HOOK")"
    [ "$status" -eq 0 ]
  done
  run bash -c "printf '%s' $(printf '%q' "{\"tool_input\":{\"file_path\":\"$REPO/docs/a.md\"}}") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 0 ]
}
...
@test "P-20: Write, MultiEdit and Edit all reach the same verdict; tool_input with no file_path key passes" {
  ...
  [ "$status" -eq 0 ]
  fp="$REPO/docs/a.md"
  run bash -c "printf '%s' $(printf '%q' "{\"tool_name\":\"Write\",\"tool_input\":{\"file_path\":\"$fp\",\"content\":\"x\"}}") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 2 ]
  run bash -c "printf '%s' $(printf '%q' "{\"tool_name\":\"MultiEdit\",\"tool_input\":{\"file_path\":\"$fp\",\"edits\":[]}}") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 2 ]
  run bash -c "printf '%s' $(printf '%q' "{\"tool_name\":\"Edit\",\"tool_input\":{\"file_path\":\"$fp\",\"old_string\":\"a\",\"new_string\":\"b\"}}") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 2 ]
  run bash -c "printf '%s' '{\"tool_name\":\"Edit\",\"tool_input\":{}}' | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 0 ]
}
...
@test "P-15: no code leakage — a python core that dies without its handler still translates to exactly 2 / 0" {
  ...
  [ "$status" -eq 0 ]
  mkstub_python3 69
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/docs/a.md") | PATH=\"$STUBDIR:\$PATH\" FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 2 ]
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/docs/a.md") | PATH=\"$STUBDIR:\$PATH\" FFS_COORD_MODE=audit CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 0 ]
}
...
@test "cache: differential equivalence across five cache states, blocked and unblocked paths" {
  ...
  mkdir -p "$REPO/other"
  # warm the cache
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/other/x.md") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") FFS_RUN_ID=other bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 0 ]
...
" "$STORE/guard-cache.json" "$STORE/registry.json"
  check_both
}
...
import json
json.dump({'tag': $tag, 'registry_sha256': '0'*64, 'exclusive_index': []}, open(sys.argv[1], 'w'))
" "$STORE/guard-cache.json"
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$fp") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") FFS_RUN_ID=other bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 2 ]
  [[ "$output" != *"COORD-GATE-FAIL"* ]]
...
import json
json.dump({'tag': $tag, 'exclusive_index': []}, open(sys.argv[1], 'w'))
" "$STORE/guard-cache.json"
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$fp") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") FFS_RUN_ID=other bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 2 ]
  [[ "$output" != *"COORD-GATE-FAIL"* ]]
...
@test "cache: write refused when guard-cache.json is a symlink — verdict-invisible, enforce+audit x blocked+unblocked" {
  ...
  [ "$status" -eq 0 ]
  [[ "$output" == *"COORD-AUDIT"* ]]
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/docs/a.md") | FFS_COORD_MODE=audit CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") FFS_RUN_ID=other bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 0 ]
...
@test "cache: written atomically inside the store — 10 concurrent invocations leave no torn cache and no temp files" {
  ...
  for i in $(seq 1 10); do
    ( bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/other/x-$i.md") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") FFS_RUN_ID=r$i bash $(printf '%q' "$HOOK")" >/dev/null 2>&1 ) &
    pids+=("$!")
  done
  for p in "${pids[@]}"; do
    wait "$p" || true
...
import json; json.load(open(sys.argv[1]))" "$STORE/guard-cache.json"
  [ "$status" -eq 0 ]
...
@test "cache: the guard writes nothing else — registry.json, sessions/, and registry.lock are untouched by a mixed workload" {
  ...
  mkdir -p "$REPO/other"
  for i in 1 2 3 4 5; do
    run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/docs/a.md") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") FFS_RUN_ID=w$i bash $(printf '%q' "$HOOK")"
    run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/other/x-$i.md") | FFS_COORD_MODE=enforce CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") FFS_RUN_ID=w$i bash $(printf '%q' "$HOOK")"
  done
...
@test "REQ-09: FFS_COORD_MODE=off never launches python" {
  ...
  run acquire_as holder "path:docs/**" exclusive
  [ "$status" -eq 0 ]
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/docs/a.md") | PATH=\"$STUBDIR:\$PATH\" FFS_COORD_MODE=off CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 0 ]
  [ ! -e "$MARKER" ]
}
...
@test "REQ-09: a mode file of 'off' never launches python" {
  ...
  mkstub_python3 1 "$MARKER"
  run bash -c "$(declare -f envelope); envelope Edit $(printf '%q' "$REPO/docs/a.md") | PATH=\"$STUBDIR:\$PATH\" CLAUDE_PROJECT_DIR=$(printf '%q' "$REPO") bash $(printf '%q' "$HOOK")"
  [ "$status" -eq 0 ]
  [ ! -e "$MARKER" ]
  rm -f "$STORE/mode"
}
